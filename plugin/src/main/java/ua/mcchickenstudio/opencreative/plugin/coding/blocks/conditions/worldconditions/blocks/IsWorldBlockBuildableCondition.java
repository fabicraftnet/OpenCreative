...
package ua.mcchickenstudio.opencreative.plugin.coding.blocks.conditions.worldconditions.blocks;
...
import ua.mcchickenstudio.opencreative.plugin.coding.blocks.conditions.worldconditions.WorldCondition;
import ua.mcchickenstudio.opencreative.plugin.coding.blocks.executors.Executor;

import java.util.List;

public final class IsWorldBlockBuildableCondition extends WorldCondition {

    public IsWorldBlockBuildableCondition(Executor executor, Target target, int x, Arguments args, List<Action> actions, List<Action> reactions, boolean isOpposed) {
    ...
    public boolean check() {
        ...
        for (Location location : blockLocations) {
            if (getWorld().getBlockAt(location).isBuildable()) {
                if (!requireAll) {
                    return true;
                }
                isBuildable = true;
    ...
    public @NotNull ActionType getActionType() {
        return ActionType.IF_WORLD_BLOCK_IS_BUILDABLE;
    }
}
