...
public final class WorldBlockIsPreferredToolCondition extends WorldCondition {

    public WorldBlockIsPreferredToolCondition(Executor executor, Target target, int x, Arguments args, List<Action> actions, List<Action> reactions, boolean isOpposed) {
    ...
    public boolean check() {
        ...
        for (Location location : blockLocations) {
            if (getWorld().getBlockAt(location).isPreferredTool(tool)) {
                if (!requireAll) {
                    return true;
                }
                isPreferred = true;
