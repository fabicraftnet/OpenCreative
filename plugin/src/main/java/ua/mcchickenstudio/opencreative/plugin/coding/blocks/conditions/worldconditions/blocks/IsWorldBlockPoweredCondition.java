...
public final class IsWorldBlockPoweredCondition extends WorldCondition {
    ...
    public boolean check() {
        ...
        for (Location location : blockLocations) {
            if (getWorld().getBlockAt(location).isBlockPowered()) {
                if (!requireAll) {
                    return true;
                }
                isPowered = true;
    ...
    public @NotNull ActionType getActionType() {
        return ActionType.IF_WORLD_BLOCK_IS_POWERED;
    }
}
