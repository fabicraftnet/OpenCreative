...
public final class AddCampfireRecipeAction extends WorldAction {
    public AddCampfireRecipeAction(Executor executor, Target target, int x, Arguments args) {
    ...
    protected void execute() {
        ...
        int cookingTime = getArguments().getInt("time", 600, this);
        if (cookingTime < 1) {
            cookingTime = 1;
        }
        if (cookingTime > 6000) {
        ...
        CampfireRecipe recipe = new CampfireRecipe(key, result, item.getType(), experience, cookingTime);
        getPlanet().getTerritory().getRecipes().addRecipe(recipe.getKey(), recipe);
    }

    @Override
    public @NotNull ActionType getActionType() {
        return ActionType.WORLD_ADD_CAMPFIRE_RECIPE;
    }
}
