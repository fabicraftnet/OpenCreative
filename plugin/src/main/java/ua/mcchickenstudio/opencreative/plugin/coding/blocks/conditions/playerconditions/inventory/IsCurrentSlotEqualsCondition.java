...
public final class IsCurrentSlotEqualsCondition extends PlayerCondition {

    public IsCurrentSlotEqualsCondition(Executor executor, Target target, int x, Arguments args, List<Action> actions, List<Action> reactions, boolean isOpposed) {
    ...
    public boolean checkPlayer(@NotNull Player player) {
        ...
        for (double slot : slots) {
            if (player.getInventory().getHeldItemSlot() + 1 == slot) return true;
        }
        return false;
    }
    ...
    public @NotNull ActionType getActionType() {
        return ActionType.IF_PLAYER_CURRENT_SLOT_EQUALS;
    }
}
