...
import ua.mcchickenstudio.opencreative.plugin.coding.blocks.events.player.ClickedInventoryEvent;
import ua.mcchickenstudio.opencreative.plugin.coding.blocks.executors.Executor;

import java.util.List;

public final class IsClickedInOwnInventoryCondition extends PlayerCondition {

    public IsClickedInOwnInventoryCondition(Executor executor, Target target, int x, Arguments args, List<Action> actions, List<Action> reactions, boolean isOpposed) {
        super(executor, target, x, args, actions, reactions, isOpposed);
    }

    @Override
    public boolean checkPlayer(@NotNull Player player) {
        if (!(getEvent() instanceof ClickedInventoryEvent clickEvent)) return false;
        if (clickEvent.getClickedInventory() == null) return false;
        InventoryHolder playerInventory = player.getInventory().getHolder();
        if (playerInventory == null) return false;
        return playerInventory.equals(clickEvent.getClickedInventory().getHolder());
    }

    @Override
    public @NotNull ActionType getActionType() {
        return ActionType.IF_PLAYER_IS_INVENTORY_OWN;
    }
}
