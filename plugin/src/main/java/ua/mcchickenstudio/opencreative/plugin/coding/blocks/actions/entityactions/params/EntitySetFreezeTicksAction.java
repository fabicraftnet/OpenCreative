...
package ua.mcchickenstudio.opencreative.plugin.coding.blocks.actions.entityactions.params;
...
public final class EntitySetFreezeTicksAction extends EntityAction {
    public EntitySetFreezeTicksAction(Executor executor, Target target, int x, Arguments args) {
    ...
    public void executeEntity(@NotNull Entity entity) {
        ...
        if (!add) {
            livingEntity.setFreezeTicks(ticks);
        } else {
            livingEntity.setFreezeTicks(livingEntity.getFreezeTicks() + ticks);
        }
    }

    @Override
    public @NotNull ActionType getActionType() {
        return ActionType.ENTITY_SET_FREEZE_TICKS;
    }
}
